...
for (l in 1:4){   # Loop through all four (4) leagues
  ...
  write.csv(giro_table_master, file = paste("giro_table_master_", league_code, ".csv", sep = ""), sep = ",", row.names = FALSE, col.names = TRUE)
  
  
}   # End loop through all four leagues
#####################
