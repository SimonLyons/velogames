...
league_codes <- read.csv("2016_vuelta_velogames_leagues.csv")

# Step 2: For each league, extract the details for team, directeur and team_weblink
for(s in 1:3){   # Loop through each of the three (3) leagues
  league_code <- league_codes$league_no[s]
  velo_url <- paste0("https://www.velogames.com/vuelta-a-espana/2016/leaguescores.php?league=", league_code)
...
for (l in 2:3){   # Loop through all three (3) leagues
  ...
  for (t in 1:n_teams){   # Loop through each of the 11 teams
    ...
    for (s in 1:22){   # Loop through all of the stages for each team. Stage 22 gives bonus points!
      team_url <- paste("https://www.velogames.com/vuelta-a-espana/2016/", team_link, "&ga=13&st=", s, sep = "")
  ...
  write.csv(tdf_table_master, file = paste("vuelta_table_master_", league_code, ".csv", sep = ""), sep = ",", row.names = FALSE, col.names = TRUE)
