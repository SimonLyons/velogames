---
title: "Vuelta Velogames Daily Update"
...
league_codes <- read.csv("2017_vuelta_velogames_leagues.csv", stringsAsFactors = FALSE)
...
<br>
<br>
A big Happy Fathers' Day shout out. I hope you find a way to enjoy a fabulous day with your kids or do something special with your Dad. Stage 14 was signature Vuelta mountain goat profile with the first out-of-category climb of the Vuelta taking the riders to a finish at 1,800m. 
<br>
<br>
Bora-Hansgrohe team leader Rafal Majka, who lost buckets of time during the first week and is out of GC contention, was the sole member of the breakaway to prevail, taking the stage victory with a small gap over a hard-chasing group of GC favourites. All the main action of the stage was behind him during the last 5km of the climb to La Pandera. Bardet and Chaves tooks turns to attack but were eventually gapped and lost time to Froome. Nibali and Contador brought the real fight and took off in the hunt for Majka in addition to building a time gap over Froome. But the Sky juggernaut stuck stoutly to their normal gameplan of riding tempo (and presumably to wattage) and eventually cynched off the advantage. Big names including Fabio Aru and Roman Bardet were shed in the process. Aru's team mate Miguel Angel Lopez launched a searing attack inside the last 2km and crossed the line in second place. He was shortly followed by a small group containing Nibali, Froome, Zakarin and Kelderman, with Contador just a few seconds later.  
<br>
Time lost by Chaves means the Orica-Scott team leader dropped from third to fifth, with Sunweb captain Kelderman now in third, Zakarin fourth. Expect more of the same GC fireworks tonight with the short Stage 15 finishing at an even higher altitude. Riders will battle up the 2,510m summit of the Sierra Nevada ski resort. Woo hoo!
<br>
<br>
Results on cycling news:
<br>
<br>
`r paste("http://www.cyclingnews.com/vuelta-a-espana/stage-", stage_no, "/results/", sep = "") `
<br>  
<br>
<br>
...
colnames(vuelta_table_master) <- gsub("\n", "", colnames(vuelta_table_master))
...
rider_stage_score <- vuelta_table_master %>% 
...
for(r in 1:length(rider_stage_score$rider)){
  rider_stage_score[r, 4] <- vuelta_table_master %>% 
...
# Build a table with unique riders, times selected, total points and cost
  ...
  rider_times_selected <- vuelta_table_master %>% 
    filter(stage == 1) %>% 
...
# This dplyr subset looks at the total points for each
# rider for the stage and lists them in descending order.
team_stage_score <- vuelta_table_master %>% 
  filter(stage == stage_no) %>% 
...
``` {r, echo=FALSE, warning=FALSE, message=FALSE}
  ...
  rider_list <- vuelta_table_master %>% 
    filter(stage == 1) %>% 
    group_by(rider) %>% 
  ...
  directeur_list <- vuelta_table_master %>% 
    select(directeur) %>% 
    distinct() %>% 
    arrange(directeur)
...
for (d in 1: length(riders_DNF$Name)){
  riders_DNF[d , 5] <- vuelta_table_master %>% 
